...
if ngem:
    ...
    start_tof = 20000  # start of the ToF range
    end_tof = 85000  # end of the ToF range
    bin_width = 10  # TOF bin width in us
    L = 56.4  # moderator to detector distance

    def get_file_list(folder="", max_files=None):
        tmp_list = list(Path(folder).glob("*/*.edb"))
        if max_files is not None:
            tmp_list = tmp_list[:max_files]
        file_list = ""
        for ii in tmp_list:
            file_list += f"{ii}, "
        return file_list[:-2]
    ...
    # auto filename
    filename = Path(ngem_path).name + "_rpt" + ".nxs"
    print("Saving:", filename)

    file_list = get_file_list(ngem_path)
    ws = LoadNGEM(Filename=file_list, Binwidth=bin_width / 2)
    ws = Rebin(ws, f"{start_tof},{bin_width},{end_tof}", PreserveEvents=False)
    ...
    def run_recon(image_stack, settings=None):
        ...
        if DEBUG:
            ...
            # Convert to uint8 for visualization
            img_normalized = (
                        (middle_slice - middle_slice.min()) * (255 / (middle_slice.max() - middle_slice.min()))).astype(
                np.uint8)
            ...
            cv2.putText(viz_img, f"Algorithm: {settings['algorithm']}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
                        2)
    ...
    output = str(output_dir.basename)
    print("Stack saved to %s", output)
