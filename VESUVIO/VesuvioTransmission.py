...
class VesuvioTransmission(PythonAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(
            ...
            doc="The root-name for the output workspaces",
        )
        self.declareProperty(
            "Runs",
            "",
        ...
        self.declareProperty(
            "Target", "Energy", StringListValidator(["Energy", "Wavelength"])
        )
    ...
    def PyExec(self):
        ...
        if grouping == "SumOfAllRuns":
            ...
            RebinToWorkspace(
                WorkspaceToRebin="empty_monitors",
                WorkspaceToMatch="sample_monitors",
                PreserveEvents=True,
                OutputWorkspace="empty_monitors",
            )
            Divide(
                LHSWorkspace="sample_monitors",
                RHSWorkspace="empty_monitors",
            ...
            if reb_bool:
                if target == "Energy":
                    Rebin(
                        InputWorkspace=name,
                        Params=reb_parameters,
                        OutputWorkspace=name,
                        FullBinsOnly=True,
                        PreserveEvents=True,
                    )
            ExtractSingleSpectrum(
                InputWorkspace=name, OutputWorkspace="tmp", WorkspaceIndex=0
            )
            ExtractSingleSpectrum(
                InputWorkspace=name, OutputWorkspace=name, WorkspaceIndex=1
            )
            RebinToWorkspace(
                WorkspaceToRebin="tmp", WorkspaceToMatch=name, OutputWorkspace="tmp"
            )
            if not invert:
                if smooth:
            ...
            for runs in range(lower + 1, upper + 1):
                ...
                ExtractSingleSpectrum(
                    InputWorkspace="tmp", OutputWorkspace="tmp2", WorkspaceIndex=0
                )
                ExtractSingleSpectrum(
                    InputWorkspace="tmp", OutputWorkspace="tmp", WorkspaceIndex=1
                )
                RebinToWorkspace(
                    WorkspaceToRebin="tmp2",
                    WorkspaceToMatch="tmp",
                    OutputWorkspace="tmp2",
                )
                Divide(LHSWorkspace="tmp", RHSWorkspace="tmp2", OutputWorkspace="tmp")
                AppendSpectra(
                    InputWorkspace1=name, InputWorkspace2="tmp", OutputWorkspace=name
                )
            DeleteWorkspace("tmp2")
        if calculate_cross_section:
            Logarithm(InputWorkspace=name, OutputWorkspace=name + "_XS")
            Scale(
                InputWorkspace=name + "_XS",
                Factor=-1,
                Operation="Multiply",
                OutputWorkspace=name + "_XS",
            )
            Integration(
                InputWorkspace=name + "_XS",
                RangeLower=1000,
            ...
            Divide(
                LHSWorkspace=name + "_XS",
                RHSWorkspace="tmp",
                OutputWorkspace=name + "_XS",
            )
            Scale(
                InputWorkspace=name + "_XS", Factor=10000, OutputWorkspace=name + "_XS"
            )
